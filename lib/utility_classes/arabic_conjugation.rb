class ArabicConjugation
  ...
  def unvocalized_arabic 
    self.consonants.flatten.compact.map { |letter| ArabicLetter.find_by_english_symbol(letter).ascii_value.to_i.chr }.join("")
  end

  def vocalized_arabic
    self.consonants.zip(vowels).flatten.compact.map { |letter| ArabicLetter.find_by_english_symbol(letter).ascii_value.to_i.chr }.join("")
  end
